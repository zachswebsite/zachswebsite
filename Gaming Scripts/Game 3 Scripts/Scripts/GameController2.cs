...
/*
    Copy of GameController, with only a few changes at Start() making player launch into air.
*/
public class GameController2 : MonoBehaviour
{
    ...
    public JumpKingScript2 playerScript;
    ...
    void Update()
    {
        ...
        if(rain.isEmitting){
            ...
            rainEmitter.rateOverTime = (float)Mathf.Round((float)(25 - (.3 * (player1.transform.position.y+4))));
...
private IEnumerator SwitchAnimations2(){
    ...
    public void checkCheckpoints(){
        if(player1.transform.position.y < 16.2 && currentCheckpoint == 1){
            //teleport player back above fireplace
            player1.transform.position = new Vector2(.51f,16.9f);
