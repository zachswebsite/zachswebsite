...
public class ShadowCasterSprite : MonoBehaviour
{
    ...
    {
        foreach (var item in FindObjectsOfType<ShadowCasterSprite>())
        {
            item.Rebuild();
        }
    }
    ...
    private void UpdateCompositeShadow(ShadowCaster2D caster)
    {
        ...
        foreach(Vector2 vec in points){
            points[i] += new Vector2(-2.35f,-3.3f);
            i++;
        }
