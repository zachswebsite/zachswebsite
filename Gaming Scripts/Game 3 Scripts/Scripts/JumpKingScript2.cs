...
/*
    Copy of JumpKingScript, needed a copy because this is a new scene, so all those old scripts are deleted.
*/
public class JumpKingScript2 : MonoBehaviour
{  
    ...
    public bool fixedStartPos;

void Start()
    {
        if(fixedStartPos){
            transform.position = new Vector2(.82f,-5.38f);
        }
        checkPointCount = 0;
        extraHeight = 0.05f;
        jumping = false;
...
void Update()
    {
        ...
        if(transform.position.y <= -5.4){
            //transform.position += new Vector3(Time.deltaTime/1.5f,Time.deltaTime/1.5f,0);
            rb.AddForce(Vector2.up * jumpMultiplier * maxJumpValue);
            rb.AddForce(Vector2.left * horizJumpSpeed);
            //preform jump
        }
