...
using System;
/* this script was gotten online
    it creates shadowboxes for all the tiles in the tilemap, based on each of their bounds, 
    Zach Willson had to change the code a little (line 138-144) in order to get it working correctly
*/
[ExecuteInEditMode]
...
public class ShadowCaster2DFromComposite : MonoBehaviour
{
    ...
    private void UpdateCompositeShadow(ShadowCaster2D caster)
    {
        ...
        foreach(Vector2 vec in points){
            points[i] += new Vector2(.06f,-1.64f);
            i++;
        }
        //and added code ends here
